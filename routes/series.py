...
def wanted():
    ...
    if (
        not cached
        and not app_state.wanted_refresh_in_progress
    ...
    for item in cached:
        ...
        if item["series_id"] not in series_stats_dict:
            series_stats_dict[item["series_id"]] = {"series": series_data, "missing": 0}
    ...
    upcoming = [
        ...
        if w["issue"].get("store_date") and w["issue"]["store_date"] > today
    ]
    missing = [
        w
        for w in wanted_issues
...
def series_search():
    ...
    return render_template("series_search.html", metron_configured=metron_configured)
...
def publishers_page():
    ...
    return render_template(
        "publishers.html", publishers=publishers, total_publishers=len(publishers)
    )
...
def issue_view(slug):
    ...
    try:
        ...
        if metron.is_connection_error(e):
            app_logger.warning(
                f"Metron unavailable while resolving issue {issue_id}: {e}"
            )
...
def series_view(slug):
    ...
    if not force_refresh:
        ...
        app_logger.info(
            f"Force refresh requested for series {series_id}, skipping cache"
        )

    # Use cache if series exists in DB (subscribed series are kept fresh by scheduled sync)
    use_cache = bool(cached_series)
    ...
    if not use_cache:
        ...
        if not api:
            if cached_series:
                app_logger.warning(
                    f"API not available, using stale cache for series {series_id}"
                )
                use_cache = True
            else:
                flash("Metron API not configured", "error")
    ...
    try:
        if use_cache and cached_series:
            ...
            app_logger.info(
                f"Got series_info: {series_info.name if series_info else 'None'}"
            )
            ...
            if hasattr(series_info, "model_dump"):
                ...
                series_dict_for_save = {
                    "id": series_id,
                    "name": getattr(series_info, "name", ""),
                }
            ...
            save_series_mapping(
                series_dict_for_save, existing_mapping or None, cover_image
            )
        ...
        if mapped_path and os.path.isdir(mapped_path):
            issue_status = match_issues_to_collection(
                mapped_path, all_issues, series_info
            )
                ...
                try:
                    if hasattr(series_info, "model_dump"):
                        ...
                        series_dict = json.loads(
                            json.dumps(series_info.dict(), default=str)
                        )
                    elif hasattr(series_info, "__dict__"):
                        import json

                        series_dict = json.loads(
                            json.dumps(vars(series_info), default=str)
                        )
                except Exception as e:
        ...
        if metron.is_connection_error(e):
            app_logger.warning(
                f"Metron unavailable while loading series {series_id}: {e}"
            )
            flash("Metron is currently unavailable. Please try again later.", "error")
        else:
            import traceback
...
def api_search_series():
    ...
    try:
        ...
        for series in results:
            ...
            series_name = getattr(series, "display_name", "") or getattr(
                series, "name", ""
            )
        ...
        return jsonify(
            {"success": True, "series": series_list, "count": len(series_list)}
        )
    except Exception as e:
        if metron.is_connection_error(e):
...
def subscribe_series(series_id):
    ...
    try:
        ...
        if success:
            if not cv_id:
                app_logger.warning(
                    f"Created cvinfo without ComicVine ID for series {series_id} at {cvinfo_path}"
                )
            else:
...
def check_series_collection(series_id):
    ...
    try:
        ...
        if cached_series and cached_issues:
            ...
            if not api:
                return jsonify(
                    {"error": "Metron API not configured and no cached data"}
                ), 500
        ...
        issue_status = match_issues_to_collection(
            mapped_path, all_issues, series_info, use_cache=not refresh
        )
        ...
        manual_not_found = sum(
            1
            for num in manual_status.keys()
        ...
        if metron.is_connection_error(e):
            app_logger.warning(
                f"Metron unavailable while checking collection for series {series_id}: {e}"
            )
            return jsonify(
                {"error": "Metron is currently unavailable. Please try again later."}
            ), 503
...
@series_bp.route(
    "/api/series/<int:series_id>/issue/<issue_number>/manual-status", methods=["POST"]
)
...
@series_bp.route(
    "/api/series/<int:series_id>/issue/<issue_number>/manual-status", methods=["DELETE"]
)
    ...
    try:
        ...
        for series in series_to_sync:
            ...
            try:
                ...
                error_msg = (
                    "Metron is currently unavailable"
                    if metron.is_connection_error(e)
                    else str(e)
                )
                log = (
                    app_logger.warning
                    if metron.is_connection_error(e)
                    else app_logger.error
                )
                log(f"Error syncing series {series_id}: {e}")
                results.append(
                    {"series_id": series_id, "success": False, "error": error_msg}
                )
...
def get_wanted_issues_api():
    ...
    try:
        ...
        return jsonify({"success": True, "issues": wanted, "count": len(wanted)})
    except Exception as e:
...
def api_refresh_wanted():
    ...
    try:
        if app_state.wanted_refresh_in_progress:
            return jsonify({"success": True, "message": "Refresh already in progress"})

        threading.Thread(target=refresh_wanted_cache_background, daemon=True).start()
        return jsonify({"success": True, "message": "Wanted issues refresh started"})
    except Exception as e:
    ...
    try:
        ...
        if library_id:
            ...
            def rebuild_index_for_new_library():
                try:
                    app_logger.info(
                        f"Rebuilding file index after adding library: {name}"
                    )
                    filesystem_entries = scan_filesystem_for_sync()
                    sync_file_index_incremental(filesystem_entries)
                    app_logger.info(
                        f"File index rebuilt successfully for new library: {name}"
                    )
                except Exception as e:
            ...
            return jsonify(
                {
                    "success": False,
                    "error": "Failed to add library (path may already exist)",
                }
            ), 400
    except Exception as e:
...
def api_update_library(library_id):
    ...
    try:
        if update_library(library_id, name=name, path=path, enabled=enabled):
            return jsonify({"success": True, "message": "Library updated successfully"})
        else:
...
def api_delete_library(library_id):
    ...
    try:
        if delete_library(library_id):
            return jsonify({"success": True, "message": "Library deleted successfully"})
        else:
...
def api_get_publishers():
    ...
    try:
        publishers = get_all_publishers()
        return jsonify({"success": True, "publishers": publishers})
    except Exception as e:
