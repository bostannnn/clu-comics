...
VOLUME_SUBTITLE_PATTERN = re.compile(
    r"^(.*?)\s+(v\d{1,3})\s+(-\s*[^-]+.*?)(\.\w+)$", re.IGNORECASE
)
...
TITLE_YEAR_PATTERN = re.compile(
    r"^(.*?)\b((?:19|20)\d{2})\b(.*)(\.\w+)$", re.IGNORECASE
)
...
# -------------------------------------------------------------------
FALLBACK_PATTERN = re.compile(r"^(.*?)\((\d{4})\)(.*)(\.\w+)$", re.IGNORECASE)
...
ISSUE_YEAR_PARENTHESES_PATTERN = re.compile(
    r"^(.*?)\s*\((\d{1,3})\s+(\d{4})\)(.*)(\.\w+)$", re.IGNORECASE
)
...
TITLE_COMMA_YEAR_ISSUE_PATTERN = re.compile(
    r"^(.*?),\s*(\d{4})-\d{2}-\d{2}\s*\(\s*(_?\d{1,3})\s*\)(.*)(\.\w+)$", re.IGNORECASE
)
...
TITLE_COMMA_YEAR_HASH_ISSUE_PATTERN = re.compile(
    r"^(.*?),\s*(\d{4})-\d{2}-\d{2}\s*\(\s*(#\d{1,4})\s*\)(.*)(\.\w+)$", re.IGNORECASE
)
...
SERIES_YEAR_MONTH_DAY_ISSUE_PATTERN = re.compile(
    r"^(.*?)\s+(\d{4})-\d{2}-\d{2}\s*\(\s*(\d{1,3})\s*\)(.*)(\.\w+)$", re.IGNORECASE
)
...
def extract_comic_values(filename):
    ...
    if international_annual_match:
        series_name, volume, annual, issue_num, year, extra, extension = (
            international_annual_match.groups()
        )
    ...
    if series_number_issue_year_match:
        series_name, issue_num, year, extra, extension = (
            series_number_issue_year_match.groups()
        )
    ...
    if series_volume_issue_yearmonth_match:
        series_name, volume, issue_num, yearmonth, extension = (
            series_volume_issue_yearmonth_match.groups()
        )
        values["series_name"] = smart_title_case(series_name.replace("_", " ").strip())
    ...
    yearmonth_series_issue_empty_match = re.match(
        r"^(\d{6})\s+(.*?)\s+(\d{1,4})\s+\(\s*\).*(\.\w+)?$", filename, re.IGNORECASE
    )
    if yearmonth_series_issue_empty_match:
        yearmonth, series_name, issue_num, extension = (
    ...
    yearmonth_series_issue_match = re.match(
        r"^(\d{6})\s+(.*?)\s+(\d{1,4})(\.\w+)?$", filename, re.IGNORECASE
    )
    if yearmonth_series_issue_match:
        yearmonth, series_name, issue_num, extension = (
            yearmonth_series_issue_match.groups()
        )
    ...
    year_month_series_volume_issue_match = YEAR_MONTH_SERIES_VOLUME_ISSUE_PATTERN.match(
        filename
    )
    if year_month_series_volume_issue_match:
        year_month, series_name, volume, issue_num, extension = (
            year_month_series_volume_issue_match.groups()
        )
    ...
    if series_year_month_issue_match:
        series_name, year, issue_num, extra, extension = (
            series_year_month_issue_match.groups()
        )
    ...
    series_year_month_day_issue_match = SERIES_YEAR_MONTH_DAY_ISSUE_PATTERN.match(
        filename
    )
    if series_year_month_day_issue_match:
        series_name, year, issue_num, extra, extension = (
            series_year_month_day_issue_match.groups()
        )
    ...
    if series_year_month_simple_match:
        series_name, year, issue_num, extra, extension = (
            series_year_month_simple_match.groups()
        )
    ...
    title_comma_year_hash_issue_match = TITLE_COMMA_YEAR_HASH_ISSUE_PATTERN.match(
        filename
    )
    if title_comma_year_hash_issue_match:
        raw_title, year, hash_issue_num, extra, extension = (
            title_comma_year_hash_issue_match.groups()
        )
        values["year"] = year
    ...
    if title_comma_year_issue_match:
        raw_title, year, issue_num, extra, extension = (
            title_comma_year_issue_match.groups()
        )
        values["year"] = year
        values["series_name"] = smart_title_case(raw_title.replace("_", " ").strip())
...
def rename_comic_from_metadata(file_path, metadata):
    ...
    try:
        ...
        new_name = apply_custom_pattern(values, custom_pattern)
        if not new_name:
            return file_path, False
        ...
        app_logger.error(f"Error during auto-rename: {e}")
        return file_path, False
...
def get_renamed_filename(filename, file_path=None):
    ...
    try:
        ...
        if custom_enabled and custom_pattern:
            ...
            app_logger.info(
                "Custom rename pattern disabled or not configured, using default logic"
            )
    except Exception as e:
        app_logger.warning(
            f"Error in custom rename logic: {e}, falling back to default logic"
        )
    ...
    if year_month_series_volume_issue_match:
        app_logger.info(
            f"Matched YEAR_MONTH_SERIES_VOLUME_ISSUE_PATTERN for: {filename}"
        )
        year_month, series_name, volume, issue_num, extension = (
            year_month_series_volume_issue_match.groups()
        )
    ...
    if vol_issue_match:
        ...
        if found_year:
            new_filename = (
                f"{clean_title} {final_volume} {final_issue} ({found_year}){extension}"
            )
        else:
    ...
    if series_match:
        ...
        if found_year:
            return clean_final_filename(
                f"{clean_title} {final_issue} ({found_year}){extension}"
            )
        ...
        for filename in files:
            ...
            if new_name and new_name != filename:
                ...
                if new_path != original_new_path:
                    app_logger.warning(
                        f"File already exists at destination. Using unique filename to prevent overwrite."
                    )
                ...
                try:
                    ...
                    if os.path.exists(new_path) and not os.path.exists(old_path):
                        app_logger.info(
                            f"Rename verification successful: new file exists, old file removed"
                        )
                    else:
                        app_logger.warning(
                            f"Rename verification failed: new file exists: {os.path.exists(new_path)}, old file exists: {os.path.exists(old_path)}"
                        )
